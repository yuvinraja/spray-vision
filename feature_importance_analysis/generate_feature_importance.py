"""
Generate Feature Importance Plots matching journal figure style
"""
...
def create_single_target_plot(importance_df, target, output_dir="outputs"):
    ...
    # Save
    output_path = Path(output_dir) / f"feature_importance_{target}_formatted.png"
...
def create_all_targets_grid(importance_df, output_dir="outputs"):
    ...
    # Save
    output_path = Path(output_dir) / "feature_importance_all_targets_formatted.png"
    plt.savefig(output_path, dpi=300, bbox_inches="tight")
    print(f"✓ Saved: {output_path}")
    plt.close()
...
def create_summary_table(importance_df, output_dir="outputs"):
    ...
    # Save to CSV
    output_path = Path(output_dir) / "feature_importance_summary.csv"
...
def main():
    ...
    print("Loading feature importances from separate models...")
